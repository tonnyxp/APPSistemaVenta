...
export class ProductoComponent implements OnInit, AfterViewInit {
  ...
  columnasTabla: string[] = ['nombre', 'categoria', 'existencia', 'precio', 'estado', 'acciones'];
  dataInicio: Producto[] = [];
  ...
  obtenerProductos() {
    this.productoServicio.lista().subscribe({
      next: (response) => {
        if (response.status) {
          this.dataListaProductos.data = response.data;
        } else {
  ...
  eliminarProducto(producto: Producto) {
    Swal.fire({
      ...
      if (resultado.isConfirmed) {
        this.productoServicio.eliminar(producto.productoId).subscribe({
          next: (response) => {
            if (response.status) {
